...
void compute_parallel(const vector<Data_struct>& data, const vector<double>& theta, int T,
    ...
    // one gradient for each thread, all start at zero
...
pair<vector<double>, bool> gradient_ascent_cpu_lazy(vector<double>& theta, vector<Data_struct>& data, int T, int d, int p, bool verbose, double step,  int max_iter, double eps){ //step and max_iter to be adjusted, possibly as an input of main.
    int N = data.size();
