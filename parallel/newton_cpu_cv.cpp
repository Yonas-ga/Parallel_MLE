...
void worker_cv(const vector<Data_struct>& data, const vector<double>& theta,
            int start, int end, vector<double>& my_gradient, vector<double>& my_H, int p, int d,int T, std::mutex& mtx,
    ...
    while(true){
        ...
        for (int i = start; i < end; i++) {
            add_one_family_cv(data[i], theta, my_gradient, my_H, p, d);
        }
        {
...
pair<vector<double>, bool> Newton_ascent_cpu_cv(vector<double>& theta, vector<Data_struct>& data, int T, int d, int p, bool verbose, double step,  int max_iter, double eps){ //step and max_iter to be adjusted, possibly as an input of main.
    int global_iter =0;
    int N = data.size();
    std::mutex mtx;
    ...
    for (int t = 0; t < T; t++) {
        ...
        partial_H[t].resize(p*(d-1) * p*(d-1), 0.0);
    }
    ...
    for (int t = 0; t < T; t++) {
        ...
        threads.push_back(thread(worker_cv, cref(data), cref(theta), start, end,
                                 ref(partial_grad[t]), ref(partial_H[t]), p , d, T, ref(mtx), ref(cv_start), ref(cv_done), ref(converged), ref(finished_workers), ref(global_iter)));
    }
    ...
    for (int i=0; i<max_iter;i++){
        ...
        // add gradients and hessians of each thread together
        for (int t = 0; t < T; t++) {
