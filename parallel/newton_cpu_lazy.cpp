...
void compute_sequential(const vector<Data_struct>& data, const vector<double>& theta,
    ...
    int N = data.size();
    for (int i = 0; i < N; i++) {
        add_one_family(data[i], theta, gradient, H, p,d);
    }
}
...
void worker(const vector<Data_struct>& data, const vector<double>& theta,
            int start, int end, vector<double>& my_gradient, vector<double>& my_H, int p, int d) {
    for (int i = start; i < end; i++) {
        add_one_family(data[i], theta, my_gradient, my_H, p, d);
    }
}
...
void compute_parallel(const vector<Data_struct>& data, const vector<double>& theta, int T,
    ...
    for (int t = 0; t < T; t++) {
        ...
        threads.push_back(thread(worker, cref(data), cref(theta), start, end,
                                 ref(partial_grad[t]), ref(partial_H[t]), p , d));
    }
...
pair<vector<double>, bool> Newton_ascent_cpu_lazy(vector<double>& theta, vector<Data_struct>& data, int T, int d, int p, bool verbose, double step,  int max_iter, double eps){ //step and max_iter to be adjusted, possibly as an input of main.
    int N = data.size();
